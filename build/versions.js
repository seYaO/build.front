 ...
 * @module 加版本号
 * @exports
 * @desc 根据gulp-rev改写，必须安装gulp-rev
 */
...
function transformFilename(file, options) {
    ...
    if (options.type === 'image') {
        ...
        let configPath = `${path.dirname(options.path)}/config.json`;
        let fileBuffer = fs.readFileSync(configPath);
        let data = fileBuffer ? fileBuffer.toString() : null;
        let dataJson = data ? JSON.parse(data) : null;
        let version = dataJson && dataJson[options.type] ? dataJson[options.type] : '000111';
        file.revHash = version;
    }
...
plugin.manifest = (pth, opts) => {
    ...
    let manifest = {};
    let sourceReg = /.jpg|.png|.gif|.jpeg|.webp|.bmp/;
    ...
    return through.obj((file, enc, cb) => {
        ...
        let revisionedFile = '';
        revisionedFile += sourceReg.test(file.extname) ? 'modules/' : '';
        revisionedFile += relPath(file.base, file.path);
