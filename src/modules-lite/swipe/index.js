/*****************************************************
 ...
 version:1.0
 copyright: //www.mjix.com，测试页面：//test.mjix.com
 *******************************************************/
var Side;
(function($) {
    $.fn.touchwipe = function(settings) {
        ...
        var is_y = 0;
        var $this,$thisNext,$thisCls,$thisNextCls,isshow;
        ...
        this.each(function() {
            ...
            if(!isInitExtra && extraEl){
                var realEl = $(this);
                $(extraEl).on(START_EV, function(){
                    _start.apply(realEl,arguments);
                }).on(MOVE_EV, function(){
                    _moving.apply(realEl,arguments);
                });
                $(extraEl).on(END_EV, function(){
                    _stop.apply(realEl,arguments);
                }).on(CANCEL_EV, function(){
                    _stop.apply(realEl,arguments);
                });
            }
        });
    ...
    Side={
        swipe:function(cfg){
            var self = this,
                index= 0,step,$this,$thisNext,$thisCls,$thisNextCls;
            if(!cfg.el){
                console.log("滑动节点不存在");
                return;
            }
                ...
                stop:function(result){
                    ...
                    if(Math.abs(dy)>5){
                        ...
                        if(dy>0){
                            step = 1;
                            $thisNext = $this.next();
                            $thisCls = "pt-page-rotateRoomTopOut";
                            $thisNextCls = "pt-page-rotateRoomTopIn";
                        }else{
                            step = -1;
                            $thisNext = $this.prev();
                            $thisCls = "pt-page-rotateRoomBottomOut";
                            $thisNextCls = "pt-page-rotateRoomBottomIn";
                        }
                        if(!$thisNext.length) return;
                        index +=step;
                        var ret = cfg.callback && cfg.callback.call(self,$this,$thisNext,dy);
                        if(ret === false){
                            return false;
                        }
                        $this.addClass($thisCls);
                        $thisNext.addClass($thisNextCls);
                        setTimeout(function(){
                            $this.removeClass($thisCls).removeClass("show").addClass("hide");
                            $thisNext.removeClass($thisNextCls).removeClass("hide").addClass("show");
                        }, 700);
                    }
                },
                move: function(data){
                    var step,el,$this = $(this);
