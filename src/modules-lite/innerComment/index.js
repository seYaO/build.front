(function() {
    ...
    function buildCommentItem(item, tagName) {
        tagName = tagName || 'li';
        return '<' + tagName + ' class="comm-detail">' +
            '<h4>' +
    ...
    function init() {
        ...
        $("body").delegate('image-line img, #imageListPage img', 'click', function () {
     ...
     **/
    var defConfig = {
        url: 'localfun/ajaxhelperwanle/GetLocalFunComment',
        showNum: 1
    };
    ...
    function innerComment(elem, param) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $(elem).html(buildInnerCommentList(data.list) + (data.list.length === 5 ? '<div class="view-all-comment" id="J_view_all_comment">查看全部点评</div>' : ''));
                initComment();
                init();
