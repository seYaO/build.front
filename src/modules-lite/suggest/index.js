    ...
    Suggest.prototype = {
        ...
        _init : function( U_conf ){
            ...
            var D_conf = {
                ...
                template: {
                    ...
                    '</div>',
                    /* 每条结果结构模板 */
                    item: '<div class="sug-item needsclick" data-item="{0}">' +
                    '{0}<i>{1}</i>' +
                    '</div>',
                    /* 历史记录结构模板,历史记录里的变量固定为path:0 */
                    history: '<div class="sug-item needsclick" data-item="{0}">' +
                    '{0}' +
                    '</div>'
                },
                /* 是否显示input框快速删除按钮 */
        ...
        _bindEvent : function(){
            ...
            $sug.delegate('.sug-item','click',function(e){
                ...
                var self = $(this),
                    selectedCountry;
                selectedCountry = self.attr("data-item");
                var elem;
                if(window.flagCount === 1){
                    elem = $(".add_person").find(".addEdit_country");
                }else{
                    elem = $(".edit_person").find(".addEdit_country");
                }
                elem.find("input").val(selectedCountry).css("color","#333");
                history.go(-1);
            });
            ...
            t.renderTimeout = win.setTimeout(function(){
                ...
                if( kw ){
                    ...
                    if(t.config.requestData){
                        t.localData.request(kw,function(kw,valArr){
                            t._renderSuggestList(kw,valArr,t.config.template.item);
                        });
                        return;
                    }
