...
(function($){
    ...
    Activity = {
        extraStr: "",
        dataUrl: "http://www.ly.com/dujia/AjaxHelper/activityHandler.ashx?type=ACTRESOURCE&pageindex=1&pagesize=700&platForm={PlatForm}&actid={ActivityId}&pid={CycleId}&IsSellOut={IsSellOut}&querytype={querytype}",
        ...
        dotDef: {
            anchorAttributes: '{{' +
            'var __url_detail = "http://www.ly.com/dujia/tours/"+value.LineId+"{PATH_EXT}?ak="+value.RedirectKey+"&SellType="+value.SellType; ' +
            'if(value.LineType == 0) { __url_detail = __url_detail.replace("{PATH_EXT}", ".html")}' +
            'else { __url_detail = __url_detail.replace("{PATH_EXT}", "/")}' +
            'if(value.SingleResourceId&&value.SingleResourceId!==0) { __url_detail = "http://www.ly.com/dujia/wanle/"+value.SingleResourceId+".html?"; }' +
        '}}'+
        ...
        tmplProAttr :function(data){
            ...
            var _url = 'http://www.ly.com/dujia/tours/'+data.LineId+'{PATH_EXT}?ak='+data.RedirectKey+'&SellType='+data.SellType;
            ...
            if(data.SingleResourceId&&data.SingleResourceId !== 0 ) {
                _url = "http://www.ly.com/dujia/wanle/"+data.SingleResourceId+".html?";
            }
        ...
        encryptEntertain:function(el,callback){
            ...
            if(playId&&playId!=="0"){
                var url = "http://www.ly.com/dujia/wanle/"+playId+".html";
        ...
        getPassParam:function(){
            var url = location.href,
                params = Activity.passParams || ["spm","refid"],
                paramArr = [],
                paramStr = '',
                reg,hasParam;
            //
        ...
        hideHeadAndFoot: function(){
            var dideHeadAndFoot = common.getParamFromUrl("hideHeadAndFoot");
            if(dideHeadAndFoot == 1){
                $("#header,#footer").hide();
            }
        }
    };
