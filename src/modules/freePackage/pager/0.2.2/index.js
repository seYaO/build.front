define("freePackage/pager/0.2.2/index",["freePackage/pager/0.2.2/index.css"],function(){
    ...
    **/
    ...
    Page.prototype.buildUrl = function (url) {
        ...
        url =getUrlParam("param",url);
        var current =this.current;
        url = url.replace(/("pageIndex":).*?(,|$)/i,function(a,$0,$1){ 
        return $0+current +$1    
        });
        return url ="http://www.ly.com/dujia/AjaxHelper/DynamicPackageAjax.ashx?Type=HotelList";
    }

    Page.prototype.buildAjaxObj = function () {
        if (this.ajaxObj.data) {
            var thisdata = decodeURIComponent(this.ajaxObj.data);
            var aa = thisdata;
            var bb= $.parseJSON(aa.replace("param=",""));
            bb.Filtrate.PageIndex = this.current;
            var cc = "param="+encodeURIComponent(JSON.stringify(bb));
            this.ajaxObj.data = cc;
        } else {
    ...
    /* 获取url后的参数 */
    function getUrlParam(name,last) {
        var reg = new RegExp("(^|&)" + name + "=([^&]*)(&|$)"); //构造一个含有目标参数的正则表达式对象
        var r = last.substr(1).match(reg);  //匹配目标参数
    ...
    function init() {
        var that = this;
        $("#J_hotelPager li").on("click", "a", function (e) {
            var elem = $(e.target);
    ...
    function buildPage() {
        ...
        htmlStr += htmlStrEnd;
        htmlStr = '<a class="turn-prev' + (current === 1 ? ' disabled' : '') + '" href="javascript:;">上一页</a>' + htmlStr;
        htmlStr += '<a class="turn-next' + (current === total ? ' disabled' : '') + '" href="javascript:;">下一页</a>';
    ...
    function buildJump() {
        return '<li class="jump-wrapper">第<input value="' + this.current + '" />页<button type="button">跳转</button></li>';
    }
    ...
    function initJump() {
        ...
        $('.jump-wrapper button', this.elem).on("click", function () {
            var num = parseInt($('.jump-wrapper input', this.elem).value);
            if (num === that.current) {
                return;
            }
        });
