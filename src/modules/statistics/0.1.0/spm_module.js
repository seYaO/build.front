    ...
    window.SPM_MODULE = {
        ...
        asynBind : function(cfg){
            ...
            var self = this,
                ...
                deliver = self.defcfg.cls.deliver,
                i;
            //传递模块内的链接加参数
            if(self.moduleid&&cfg.length>0){
                for(i=0;i<cfg.length;i++){
                    $(cfg[i]).each(function(){
                        ...
                        if(this.tagName.toUpperCase()==="A"){
                            ...
                            Jself.addClass(deliver);
                            Jself.find("a").not("["+moduleAttred+"]").each(function(){
                                var Jme = $(this),
                                    url = Jme.attr("href");
                                url = self.addParams(url,param,self.moduleid.toString());
                                //如果需要额外参数添加
                                if(extraAttr&&Jme.attr(extraAttr)){
                                    Jme.attr(extraAttr,url);
                                }
                                Jme.attr("href",url);
