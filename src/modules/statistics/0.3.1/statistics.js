...
(function () {
    ...
    stat._init = function (cfg) {
        ...
        window.setTimeout(function () {
            ...
            for (var key in cfg) {
                ...
                //
                if (key === "trace") {
                    self.traceExtraData = stat.extend({},cfg[key]);
                    continue;
                }
    ...
    stat.autoTrace = function(parents){
        var t0 = performance.now();
        parents = parents||$("[trace^=auto_trace]");
         parents.each(function(){
             var parent = $(this);
             var pTrace = parent.attr("trace");
             if(pTrace && pTrace.indexOf("auto_trace")>-1){
                 var autoLinks = parent.find("a,.J_trace,.j_trace").not("[trace]");
                 autoLinks.each(function(i){
                     this.setAttribute("trace","auto__"+i);
                 });
             }
         });
        var t1 = performance.now();
        //window.console && window.console.log(t1-t0);
    };
    stat.traceEl = function($el,traceVal){
            var arr=[],self=this;
            var elParents = $el.parents("[trace]"),
                parentLen = elParents.length;
            if(parentLen){
                var elTrace = traceVal||$el.attr("trace"),
                    hasHref = (($el[0].host||$el.attr("data-url"))),
                    //判断是否需要立即发送
                //当class=J_trace或者a的target=blank时,为真
                    isTargetBlank = ($el.attr("target") === "_blank" ||!hasHref||$el[0].className.toLowerCase().indexOf("j_trace")>-1);
                if(!elTrace){
                    if(parentLen >=2){
                        self.autoTrace(elParents.eq(0));
                        elTrace = $el.attr("trace");
                    }
                }
                if(elTrace){
                    //检查是否是单复杂度的,防止trace名称重复,追加父元素的trace
                    if(/^[A-z]+$|^\d+$|auto__\d+$/.exec(elTrace)){
                ...
                arr.reverse();
                self.trace.call($el,arr, self.traceExtraData);
            }
    };
    ...
    stat.getVrcode = function() {
        ...
        var _vrcode = '{NNNNN}-{NNNN}-0'.replace("{NNNNN}-{NNNN}", function() {
            ...
            //    // touch cn
            //    if ($("#isThemePage").length && parseInt($("#isThemePage").val())) {
            //        return '10004-2030';
            //    } else {
            //        return '10004-2006';
            //    }
