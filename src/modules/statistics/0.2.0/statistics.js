...
(function () {
    ...
    stat.initTrace = function(){
        var self=this;
        if(isFish) return;
        $(document).on("click","a,.J_Trace,.J_trace",function(e){
            var $el= $(this);
            var arr=[];
            ...
            if(elParents && elParents.length){
                ...
                arr.reverse();
                self.trace.apply(self,arr);
            }
        });
        //如果url包含debug,则开启埋点检测模式
        if(location.href.indexOf("debug")>-1){
            stat.addTraceCheckStyle();
        }
    };
    stat.addTraceCheckStyle = function() {
    ...
    stat.trace = function(){
            ...
            try{
                Monitor && Monitor.stat("trace",traceObj);
                _tcTraObj._tcTrackEvent.apply(_tcTraObj,args);
            }catch(e){
                Monitor.log(e.track| e.message,"trace");
            }
    };
