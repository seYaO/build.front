...
(function(module) {
    ...
    jsextend.initJQuery = function() {
        if ($) {
            ...
            //同步池子,将push进去的异步方法挨个执行
            $.SyncPools = function() {
                this.methods = new Array();
                this.isrun = false;
                this.push = function(methodhandle) {
                    this.methods.push(methodhandle);
                    if (!this.isrun) {
                        this.start();
                    }
                }
                this.start = function(req) {
                    ...
                    if (this.methods.length > 0) {
                        this.isrun = true;
                        this.methods[0](req, function(result) {
                            self.methods.removeAt(0);
                            self.start(result);
                        });
            ...
            //排队执行
            $.QueryPools = $.SyncPools;

            //异步事件池子,push进去的所有异步执行完后会触发end事件
            $.AsyncPools = function() {
                this.eventlist = new Array();
                this.count = 0;
                this.push = function(task, req) {
                    var self = this;
                    self.count++;
                    task(req, function(result) {
                        self.count--;
                        if (self.count == 0) {
                            self.trigger("end");
                        }
                ...
                this.on = function(eventName, cb) {
                    this.eventlist.push({ eventName: eventName, cb: cb });
                };
            };

            //汇总
            $.GatherPools = $.AsyncPools;
        }
    }
