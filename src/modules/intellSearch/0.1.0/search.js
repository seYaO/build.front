define('intellsearch/0.1.0/search', ['base/0.1.0/base', 'intellsearch/0.1.0/search.css', 'intellSearch/0.1.0/tmpl/search'], function (require, exports, module) {
    var searchTmpl = require('intellSearch/0.1.0/tmpl/search');
    /*
    ...
    var Search = Base.extend({
        ...
        init: function (options) {
            ...
            //var o_wapper = $();
            var _html = tmpText({ placeholder: this.get("placeholder"), submitName: this.get("submitName") });
            var o_wapper = $(self.get("el")).append(_html);

            var o_input = self.o_input = o_wapper.find("[search-input]");
            var o_group = self.o_group = o_wapper.find("[search-group]");
            ...
            $.each(modules, function (key, item) {
                ...
                if (hasPanel !== false) {
                    elobj.appendTo(o_group);
                }
                if (item.type == 'module') {
                    var config = item.config;                 
                    $.extend(config, { o_pobj: self, o_wapper: elobj, o_input: o_input });
