(function (_module) {
    if (typeof (define) != "undefined" && define.amd) {
        define(["panel/0.2.0/panel", 'datepicker/0.2.1/tmpl/calendar', 'datepicker/0.2.1/calendar-year', 'datepicker/0.2.1/calendar-month',
            "css!datepicker/0.2.1/theme/default", 'css!fonticon/0.1.0/fonticon'], function (Panel, calendarTmpl, yearPanel, monthPanel) {
                return _module(Panel, calendarTmpl, yearPanel, monthPanel);
            });
    }
    else if (typeof (define) != "undefined" && define.cmd) {
        define("datepicker/0.2.1/datepicker", ["panel/0.2.0/panel", 'datepicker/0.2.1/tmpl/calendar', 'datepicker/0.2.1/calendar-year',
            'datepicker/0.2.1/calendar-month', "datepicker/0.2.1/theme/default.css", 'fonticon/0.1.0/fonticon.css'], function (require, exports, module) {
                var Panel = require("panel/0.2.0/panel"),
                   calendarTmpl = require('./tmpl/calendar.dot'),
                    yearPanel = require('datepicker/0.2.1/calendar-year'),
                    monthPanel = require('datepicker/0.2.1/calendar-month');
                return _module(Panel, calendarTmpl, yearPanel, monthPanel);
            });
    }
    else {
        ...
        METHODS: {
            ...
            getMonthData: function (startdate) {
                ...
                for (var i = 0; i < firstweekday; i++) {
                    var _date = new Date(years, month - 1, -1 * (firstweekday - i));
            ...
            setValues: function (vs) {
                ...
                if (vs.length > 0) {
                    self.attr.value = vs[0];
                    this.resetInitDate(self.attr.value);
                }  
            },
            ...
            resetInitDate: function (date) {
                var date = date.replace(/-/g,"/");
                this.__nowDate = new Date(date);
            }
        },
        ...
        __initYearPanel: function () {
            ...
            this.o_wrapper.on('click', '.group_title .year', function () {
                ...
                if (self.attr.minDate) {
                    opt.startYear = (new Date(self.attr.minDate)).getFullYear();
                }
                else {
                    opt.startYear = 0;
                }
                if (self.attr.maxDate) {
                    opt.endYear = (new Date(self.attr.maxDate)).getFullYear();
                   
                }
                else {
                    opt.endYear = 0;
                }
                self.yearPanel.open(opt);
            });
        },
        /*
        * 初始化月选择面板
        */
        ...
        __setCalData: function (month) {
            ...
            var formatDayData = function (item) {
                var date = item.date;
                var issel = self.__values.contain(date);
                item.enable = true;
                item.tagdate = date;
