define("calendar/0.3.0/comcal",["common/0.1.0/ua",'calendar/0.3.0/festivals'],function(require,exports,module){
    var $ = jQuery,
...
// 定位增强
        ...
        //判断是否存在早订惠，有早订惠时返回月份
        function isEarlyBooking() {
            var data = window.page_cf.PriceList,
                isZaoDingHui,ret = [];

            for(var i = 0, len = data.length-1; i<=len; i++){
                var item = data[i],
                    month = item.Date.slice(0,7).replace(/-0/,"-");
                isZaoDingHui = item.IsZaoDingHui;
                if (isZaoDingHui) {
        ...
        function buildTabs(cfg){
            var html = '<div class="calendar-header"><div class="calendar-header-inner"><ul>',
                ...
                year;
            var monthArr = isEarlyBooking();

            for(var i = 0, len = cfg.length-1; i<=len; i++){
                var item = cfg[i],
                    lowerPrice = item.LowestPrice,
                    //lowerPrice（无团期）  团期全满（有团期但是团期已满）
                    priceStr = lowerPrice && item.HasResidual ? ("<em>&yen;" + lowerPrice + "/人</em>") : "团期全满",
                    aCls = "",
                    priceDateCls = "",
                    earlyBook = "",
                    year = item.Year,
                ...
                if (lowerPrice && item.HasResidual) {
                    priceDateCls = "price-date";
                }
                if(monthArr.indexOf(year + "-" + month) > -1) {
                    earlyBook = "earlyBook";
                    html += '<li class="' + aCls + ' ' + priceDateCls + ' ' + earlyBook +'" data-month="' + month + '" data-year="' + year + '"><a><span>' + month + '月</span>' + priceStr + '</a></li>';
                } else {
        ...
        function build() {
            ...
            for (var i = 0; i < this.monthNum; i++) {
                ...
                htmlStr += '<div class="calendar-wrapper" trackkit="Departure_date">' +
                        nativeTabStr+
        ...
        function init() {
            ...
            $(this.panel).on("click", function(e) {
                ...
                while (target !== this) {
                    ...
                    if (target[0].tagName.toLowerCase() === "td") {
                        ...
                        if($(target[0]).hasClass('previous-monthDay')){
                          itemMonth = parseInt(activeMonth()) -1;
                        }else if($(target[0]).hasClass('next-monthDay')){
                          itemMonth = parseInt(activeMonth()) + 1;
                        }else{
                          itemMonth = parseInt(activeMonth());
                        }
                        var successFunc = that._.fn||that._.itemClick;
                        if (typeof successFunc === "function") {
                            successFunc(date.getFullYear(), itemMonth, dateStr, target[0],e);
                        }
                        break;
            ...
            function activeMonth(){
              var noMonth;
              var tabs = $('.calendar-header-inner li');
              if(tabs.length > 0){
                for(var i = 0; i<tabs.length;i++){
                  if($(tabs[i]).hasClass('active')){
                    noMonth = $(tabs[i]).attr('data-month');
                  }
                }
              }
              return noMonth;
         ...
         * 节气节日：清明
         **/
        var festivals = require('calendar/0.3.0/festivals');
        // 其他节气和放假安排，提供接口，可以完成
        ...
        function updateMonth(wrapper, date, data) {
            ...
            while (date1.getDay() !== this.firstDay) {
                ...
                if (this.showOtherMonth) {
                    ...
                    setClassName(td, "this-month-day previous-monthDay");
                    processDate.call(this, td, date1, data);
                } else {
                    td.innerHTML = "";
            ...
            while (rowIndex < 7) {
                ...
                if (this.showOtherMonth) {
                    ...
                    setClassName(td, "this-month-day next-monthDay");
                    processDate.call(this, td, date, data);
                } else {
                    td.innerHTML = "";
                    setClassName(td, "invalid-day not-this-day");
