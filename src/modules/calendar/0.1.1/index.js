define("calendar/0.1.1/index",["calendar/0.1.1/comcal","calendar/0.1.1/index.css"],function(require,exports,module){
    var ComCal = require("calendar/0.1.1/comcal");
    var calendar ={};
    (function(){
        var $ = jQuery;
        var calendarData;
        ...
        calendar.build = function(td,date, dateStr, data, currDate) {
            ...
            if (!info.Price) {
                ...
                if(info.Residual < 0){
                    classArr.push("invalid-day");
                }else if(place.startWith("余位")||info.Residual === 0||place.startWith("成团余位")){
                    classArr.push("over");
                }else{
        ...
        calendar.init = function(param){
            ...
            self.inst = new ComCal({
                ...
                buildContent: function (td, date, dateStr) {
                    ...
                    self.build(td, date, dateStr, calendarData.priceList, currentDate);
                },
                afterRender: function () {
                    var retFlag = param.afterRender.apply(this, arguments),
                        //如果afterRender返回真，那么flag为false，则重新执行initEv
                        //如果afterRender返回非真，那么flag=self.isRenderred
                        flag = !self.isRefresh && self.isRenderred;
