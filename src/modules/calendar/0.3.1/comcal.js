define("calendar/0.3.1/comcal",["common/0.1.0/ua"],function(require,exports,module){
    var $ = jQuery,
...
// 定位增强
        ...
        function buildTabs(cfg){
            var html = '<div class="calendar-header"><div class="calendar-header-inner1"><ul>',
                isHighLight = false,
            ...
            html += '</ul></div><a onselectstart="return false;" style="-moz-user-select:none;" class="prev J_PrevMonth"><span></span></a><a class="next J_nextMonth" onselectstart="return false;" style="-moz-user-select:none;"><span></span></a></div>';
            return html;
        }
        function tabExtraBuild(isNewTab){
            ...
            if(isNewTab){
                ...
                htmlStr ='<div class="month-nav">' +
                    '<a href="javascript:;" hidefocus="true" class="previous-month J_PrevMonth"></a>' +
                    '<a href="javascript:;" hidefocus="true" class="next-month J_nextMonth"></a>' +
                    '</div>';
            }
            return htmlStr;
        }
         ...
         * 如果需要定制日历，通过样式来完成
         * wrapperWidth {number} 每个日历内容板(类名为calendar-wrapper1的div)的宽度，默认是231px((29 + 1) * 7 + 1 + 10 * 2)
        ...
        function build() {
            ...
            panel.className = newStyleSkin+"calendar-panel1" + (this.skin ? " " + this.skin : "");
            ...
            for (var i = 0; i < this.monthNum; i++) {
                //htmlStr += '<div class="calendar-wrapper1" style="width:' + this.wrapperWidth + 'px">' +
                htmlStr += '<div class="calendar-wrapper1">' +
                        nativeTabStr+
        ...
        function init() {
            var that = this,
                wrappers = $(".calendar-wrapper1", this.panel), // 通过闭包引用，下面几个也是 this.panel.getElementsByTagName("table")
                target,
                index,
                startDate,
                endDate,
            ...
            $(this.panel).on("click", function(e) {
                ...
                while (target !== this) {
                    ...
                    if (target[0].tagName.toLowerCase() === "td") {
                        ...
                        var par = $(target).parents(".calendar-wrapper1").find("h6 span"),
                            parYear = $(par).attr("data-year")||"2016",
                            parMonth = $(par).attr("data-month")||"1";
                        var curMonth = new Date(parYear+"/"+parMonth+"/1");
                        var nowMonth = curMonth||that.startDate,
                            date = nowMonth,
                        ...
                        break;
                    } else if (target.hasClass("J_PrevMonth")) {
                        if (!target.hasClass("previous-month-disabled")) {
                            that.previousMonth();
                        }
                        break;
                    } else if (target.hasClass("J_nextMonth")) {
                        ...
                        index = wrappers.indexOf(target.parent(".calendar-wrapper1")[0]);
                ...
                if (!delegateTarget.hasClass("invalid-day")) {
                    ...
                    // 如果当前日期小于开始时间，直接返回
                    var yearAndMonthStr = that.getYearAndMonthStr(delegateTarget.parent(".calendar-wrapper1")),
            ...
            $(this.yearPanel).on("click", function(e) {
                ...
                while (elem !== this) {
                    ...
                    if (tagName === "a") {
                        ...
                        } else if($(elem).hasClass("J_nextMonth")) {
                            years = this.getElementsByTagName("a");
                            year = parseInt(years[0].innerHTML, 10) + 10;
                            yearPanelBuild.call(that, year, that._.startDate, years);
                        }
            ...
            $(that.panel.getElementsByTagName("input")).on("keydown", function(e) {
                ...
                if (this.name === "year") {
                    ...
                    if (year < startDate.getFullYear() || year > that._.endDate.getFullYear()) {
                        ...
                        month = parseInt($("input", $(this).parent("calendar-wrapper1"))[1].value, 10) - 1;
        ...
        Calendar.prototype.updateHeaderMonth = function(e){
            ...
            m.attr("data-month",month);
            m.attr("data-year",year);
        };
        Calendar.prototype.update = function(year, month) {
            var date,
                wrappers = $(".calendar-wrapper1", this.panel); // this.panel.getElementsByTagName("table");
            this.today = new Date();
            ...
            if (this._.endDate) {
                if ((this._.endDate.getFullYear() === date.getFullYear() && this._.endDate.getMonth() === date.getMonth()) || this._.endDate.getTime() < date.getTime()) {
                    $(".J_nextMonth", this.panel).addClass("next-month-disabled");
                } else {
                    $(".J_nextMonth", this.panel).removeClass("next-month-disabled");
                }
            }
            this.initEvent && this.initEvent.call(this,wrappers);
        ...
        function navMonth(isPrevious) {
            ...
            var mnum = (this.monthNum===1?0:this.monthNum);
            if (isPrevious) {
                date.setMonth(date.getMonth() - mnum - 1);
            } else {
                date.setMonth(date.getMonth() + 1);
            }
            this.update(date);
