(function(_module) {
    if (typeof(define) != "undefined" && define.amd) {
        define(['base/0.3.1/react-base', 'jsextend/0.1.0/jsextend'], _module);
    } else if (typeof(define) != "undefined" && define.cmd) {
        define("base/0.3.1/react-module", ['base/0.3.1/react-base', 'jsextend/0.1.0/jsextend'], function(require, exports, module) {
            var ReactBase = require('base/0.3.1/react-base');
            return _module(ReactBase);
        });
    } else {
    ...
    var ReactModule = {
        ...
        triggerData: function(data, field) {
            field = field || this.objtree.fieldpath;
            this.objtree.__dataTrigger(field, data);
        },
        autoKey: function(i) {
    ...
    var ObjTree = function(root) {
        ...
        // }
        //初始化自己全路径
        var pfield = this.parent ? this.parent.fieldpath : "";
        ...
        this.dataChange = function(field, value) {
            self.setData(field, value);
            //以下逻辑待定
            if (self.react.dataChange) {
                self.react.dataChange(field, value);
            } else {
    ...
    //向上冒泡数据更改事件
    ObjTree.prototype.__dataTrigger = function(field, value, target) {
        //console.log("row:" + field);
        var self = this;
        if (!this.dataroot) {
            // var _field = self.parent.field ? (self.parent.field + (field ? ("." + field) : "")) : field;
            this.parent.__dataTrigger(field, value, target);
        } else {
            //没有__dataTrigger表示该组件为最外层组件
            //更改一个节点,所有子节点全都会发生变化
            // console.log(field);
            this.__dataChange(field, value);

            //数据监听
            if (this.__watchlist) {
                this.__watchlist.map(function(v, i) {
                    if (v.key == field) {
                        v.callback.call(self, target, value);
                    }
                });
            }
        }
    ...
    //向下传递数据更改
    ObjTree.prototype.__dataChange = function(field, value) {
        var self = this;
        ...
        this.fields.forEach(function(v) {
            if (field == "") { //触发源为最外层,更新所有值
                ...
                v.objtree.dataChange(field, nv);
                return;
            }
            if (v.field == "") {
                //最外层会受所有变动影响
                v.objtree.dataChange(field, value);
                return;
            }

            if (v.field == field) { //同字段
                v.objtree.dataChange("", value);
            } else if ((v.field).indexOf(field + ".") == 0) {
                //变动子集
                var nv = self.getJsonValue(v.field.substr(field.length + 1, v.field.length), value);
                // console.log("son", nv);
                v.objtree.dataChange("", nv);
            } else if ((field + ".").indexOf(v.field) == 0) {
                //变动父集
                v.objtree.dataChange(field, value);
            }
        });
