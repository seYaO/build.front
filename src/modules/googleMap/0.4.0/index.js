var GhotelLon=[126.982577][0]||100.58026; //默认的地图中心点
var GhotelLat=[37.565994][0]||13.723286; //默认的地图中心点
var map;
var poly; //地图上的线
var path = [];  //地图上线的path
...
function MyMapLoad(dom) {
    ...
    */
    ...
    MyMarker.prototype.onAdd = function () {
        ...
        $(this.div).bind("mouseenter", function () { e.mouseoverFun(e.index, e); });
        $(this.div).bind("mouseleave", function () { e.mouseoutFun(e.index, e); });
    }
    //绘制图标，主要用于控制图标的位置
    ...
    MyHotelListMap = new HotelListMap(MyMarker, InfoBox,dom);
...
function HotelListMap(MyMarker, InfoBox,dom) {

    var _mapProp = {
        //center: { lat: GhotelLat, lng: GhotelLon },
        center: { lat: pointList[0].PILatGG, lng: pointList[0].PILonGG },
        zoom: 12,
        scaleControl: true,
    ...
    map = new google.maps.Map(dom, _mapProp);
    ...
    var moveFlag = false;
    var obj = this;
    this.PointList = window.pointList;
    ...
    this.mouseoverFun = function (index, obj, isShowMyMapTip) {
        ...
        var top = screenCoor.clientY +40;
        // var left = screenCoor.clientX - 150;
        var left = screenCoor.clientX - 100;
        if (screenCoor.clientY < 0 || screenCoor.clientX < 0 ||
            ...
            if (isShowMyMapTip != "noshowmyMapTip") {
                ...
                var lMHIhtml = '<div class="littleMapHotelInfo clearfix">\
                                <div class="lMHILeft">\
                                    <img src="' + hotelImg + '">\
                                </div>\
                                <div class="lMHIRight">\
                                    <p class="lMHITitle J_mapTitle" data-pid = '+obj.pid+'>' + SubTitleByLen(obj.TitleText, 30) + '</p>\
                                    <p>' + obj.EnglishName + '</p>\
                                    <p>优质度：' + starsLevel(obj.hotelStar) + '</p>\
                                    <p>游玩时间：' + (obj.playTime||'暂无') + '</p>\
                                </div>\
                            </div>\
                            <div class="mapAddTravel" data-lat = '+obj.lat+' data-lng = '+obj.lng+' data-pid = '+obj.pid+'   data-type = '+obj.typeid+'  data-playtime = '+obj.playTime+'>添加到行程</div>';
    ...
    InitMarkers(pointList,dom);
    this.MarkLists = MarkLists;
    //初始化地图上的图标
    function InitMarkers(pointList,dom,isRemove) {
        ...
        map = new google.maps.Map(dom, mapProp);
        ...
        for (var i = 0; i <= pointList.length - 1; i++) {
            ...
            var Marker2 = new MyMarker({
                ...
                labelText: "<span  class='viewSpot point-icon'></span>",
                index: i,
                pid:item.PIPOISerialNumber,
                playTime:item.PEPlayTime,
                mouseoverFun: obj.mouseoverFun,
                mouseoutFun: obj.mouseoutFun,
                map: map,
                typeid:item.PIType1Id
            });
            ...
            Marker2.pid = item.PIPOISerialNumber;
            Marker2.playTime = item.PEPlayTime;
            Marker2.labelText = "<span  class='point-icon point-icon-"+item.PIType1Id+"'></span>"
            Marker2.typeid = item.PIType1Id;
...
//星级转换
function starsLevel(star){
    var level = ["E", "D", "C", "B", "A"]
    return level[star - 1]||'暂无';
}

function ajaxRender(data,dom){
    IndexData.thisdata = data;
    MyHotelListMap.InitMarkers(data,dom);
    // if (MyHotelListMap && MyHotelListMap != undefined) {
    //     MyHotelListMap.InitMarkers(data,dom);
    // }
}
uiMap.clickRender = function(data){
    ajaxRender(data,document.getElementById("MyMap"));
}

uiMap.clear = function(){
  for (var i = 0; i < MarkLists.length; i++) {
    MarkLists[i].setMap(null);
  }
  MarkLists = [];
}

// uiMap.initDetailMap = function(){
//     var data = {
//         'PEImageUrl':"http://pic4.40017.cn/poi/2016/04/06/14/QjMLu4.jpg",
//         'PEPlayTime':"建议2小时",
//         'PIArea2Id':0,
//         'PIId':5,
//         'PILatGG':23.615282,
//         'PILonGG':120.578115,
//         'PIName':"剑湖山世界",
//         'PINameEN':"JanfusunFancyworld",
//         'PIPOISerialNumber':"51200000000001",
//         'PIStars':5,
//         'PIType1Id':1
//     }
//     var datas = [];
//     datas.push(data);
//     ajaxRender(datas,document.getElementById('ui-sceneryMap'))
// }
