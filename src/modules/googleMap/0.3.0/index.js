...
function MyMapLoad(pointList) {
    ...
    */
    ...
    window.MyHotelListMap = MyHotelListMap = new HotelListMap(MyMarker, InfoBox, pointList);
...
function HotelListMap(MyMarker, InfoBox, pointList) {
    if(product.Data){
        var index = product.Data.HotelIndex;
        var GhotelLat = product.Data.Hotels[index].Latitude;
        var GhotelLon= product.Data.Hotels[index].Longitude;
    }else{
        return;
    }
    var mapProp = {
    ...
    this.mouseoverFun = function (index, obj, isShowMyMapTip) {
        if (obj.isShow) {
            $(".fp-item").eq(index).addClass("fp-itemActive");
            obj.IsActive = true;
            ...
            var top = screenCoor.clientY - 34 / 2 - $("#myMapTip")[0].offsetHeight / 2 + 30;
            ...
            if (screenCoor.clientY < 0 || screenCoor.clientX < 0 ||
                ...
                if (isShowMyMapTip != "noshowmyMapTip") {
                    ...
                    if (obj.hotelScore != undefined && obj.hotelScore != "0" && obj.hotelScore != "0.0") {
                        HotelScoreHtml += '<span class="lMHIScore"><span>' + obj.hotelScore + '分</span></span>';
                    }

                    var HotelAddressHtml = "";
                    if (obj.hotelAddress != undefined && obj.hotelAddress != "") {
                        HotelAddressHtml += '<p class="lMHIAddress">地址：' + SubTitleByLen(obj.hotelAddress, 30) + '</p>';
                    }
                    ...
                    var lMHIhtml = '<div class="littleMapHotelInfo">\
                                    ...
                                    <div class="lMHIRight">\
                                        ...
                                        <div class="lMHIStarScore clearfix">' + HotelScoreHtml + HotelStarHtml + '\
                                        </div>' + HotelAddressHtml + '\
                                    </div>\
                                    <div class="clear"></div>\
                                </div>';
                    $("#myMapTip").css('width','360px');
    ...
    function InitMarkers(pointList) {
        ...
        for (var i = 0; i <= pointList.length - 1; i++) {
            ...
            var Marker2 = new MyMarker({
                ...
                labelText: "<span  class='mapMarkerIcon'>"+ (i+1) +"</span>",
                index: i,
            ...
            Marker2.TitleText = item.englishname + '(' +item.name + ')';
            Marker2.hotelStar = item.hotelStar;
            Marker2.hotelScore = item.hotelScore;
            Marker2.hotelPic = item.hotelPic;
            Marker2.hotelAddress = item.hotelAddress;
