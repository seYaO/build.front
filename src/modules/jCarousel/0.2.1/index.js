function Carousel(elem, options) {
    ...
    if (options.circular && options.effect === "scroll") {
    ...
    if (!this.running) {
        ...
        this._animateToPosition({         // Animate carousel item to position based on calculated values.
            start: function () {
                ...
                if(options.circular) {
                    navIndex -= 1;
                    if(navIndex === self.initialLi.length) {
                        navIndex = 0;
                    }
                }
                self.triggerEl && self.triggerEl.eq(navIndex).addClass(self.options.selectedClass)
                    .siblings().removeClass(self.options.selectedClass);
            }
        });
...
Carousel.prototype._attachEventHandlers = function () {
    ...
    if(options.btnNav){
        $.each(self.triggerEl, function (i, el) {
            $(el).on(options.triggerType, function () {
                self.pause();
...
Carousel.defaultCfg = {
    ...
    effect: "scroll",         //滚动效果 scroll fade
    preload: 0,                //预加载
...
define("jCarousel/0.2.1/index",[],function(require, exports, module){
