...
<div class="order" v-if="initData">
    ...
    <section>
        <!-- 起保时间 start -->
        <div class="picker-line">
            <div class="picker-row" v-if="startTime['isShow']" @click="$refs.startTime.open();startTime['errMsg'] = '';mtDateIndex = -1">
            ...
            <div class="picker-row" v-else>
                <div class="picker-name picker-time"><i class="time-icon"></i><span>起保时间</span></div>
                <div class="picker-val">{{startTime['value']}}</div>
            </div>
            ...
            <mt-datetime-picker
                v-if="startTime['start'] && startTime['end']"
                v-model="startTime['default']"
                ref="startTime"
                type="date"
                :start-date="startTime['startTime']"
                :end-date="startTime['endTime']"
                year-format="{value} 年"
                month-format="{value} 月"
                date-format="{value} 日"
                @confirm="mtDateTimeConfirm"
            />
        </div>
        <!-- 起保时间 end -->
        <!-- 旅行团号 -->
    ...
    <section v-if="holder['type'] === 'appoint'">
        ...
        <template v-if="holder['isShow']">
            ...
            <div class="picker-info">
                ...
                <div class="picker-line">
                    <div class="rowLine"></div>
                    <div class="picker-row" @click="$refs.holderBirthday.open(); holder.person.birthdayErrMsg = '';mtDateIndex = -3">
                    ...
                    <mt-datetime-picker
                        v-if="holder.person['birthdayEnd']"
                        v-model="holder.person['defaultBirthday']"
                        ref="holderBirthday"
                        type="date"
                        :start-date="new Date('1900/01/01')"
                        :end-date="holder.person['birthdayEndTime']"
                        year-format="{value} 年"
                        month-format="{value} 月"
                        date-format="{value} 日"
                        @confirm="mtDateTimeConfirm"
    ...
    <section v-if="insurants && insurants[0]" v-for="(item, index) in insurants" :key="index">
        ...
        <template v-if="item['isShow']">
            ...
            <div class="picker-info">
                ...
                <div class="picker-line">
                    <div class="rowLine"></div>
                    <div class="picker-row" @click="$refs.insurantBirthday.open();DATA.birthday['default'] = item['defaultBirthday'];item.birthdayErrMsg = '';mtDateIndex = index">
                        <div class="picker-name">出生日期</div>
    ...
    <mt-datetime-picker
        v-if="DATA.birthday['startTime'] && DATA.birthday['endTime']"
        v-model="DATA.birthday['default']"
        ref="insurantBirthday"
        type="date"
        :start-date="DATA.birthday['startTime']"
        :end-date="DATA.birthday['endTime']"
        year-format="{value} 年"
        month-format="{value} 月"
        date-format="{value} 日"
        @confirm="mtDateTimeConfirm"
    ...
    <div class="agreement">
        ...
        <div class="info">
            已阅读并了解<span class="link" @click="linkHandler('clause')">保险条款</span>、<span class="link" @click="linkHandler('instruction')">保险须知</span>、<span class="link" @click="linkHandler('inform')">客户告知书</span>。<span class="gra">此产品销售服务方为：天圆地方（北京）保险代理有限公司</span>
        </div>
    </div>
    <!--  -->
    ...
    methods: {
        ...
        async initShowFields({ isEnterprise, isSingle, isMass }) {
            ...
            // 投保人
            {
                // 指定投保人
                {
                    ...
                    person.defaultBirthday = endTime;
                }
                // 企业投保
                {
            ...
            // 初始化被保人出身日期范围/priceFactors
            {
                ...
                birthday.startTime = new Date(startDate.replace(/-/, '/'));
                birthday.end = endDate;
                birthday.endTime = new Date(endDate.replace(/-/, '/'));
                birthday.default = birthday.endTime;
        ...
        getOcrInfo(index, info) {
            ...
            if(index === -1){
                ...
                Object.assign(person, {
                    ...
                    cardNo,
                    birthday,
                    defaultBirthday: new Date(birthday.replace(/-/, '/')),
                    genderType: gender,
                })
            }else{
        ...
        startTimeChange(selectDate) {
            ...
            if(maxPeriodUnit == 'O'){
                ...
                endStr = elapseDay.add('days', -1).format('yyyy年MM月dd日');
                endTemp = elapseDay.add('days', 0).format('yyyy-MM-dd');
                endStr += '24时止'
            }
            ...
            // this.startTime['default'] = selectTime;
            this.updateChoose();
        },
        ...
        appointHandler(index, obj) {
            ...
            Object.assign(this.holder.person, {
                ...
                defaultBirthday: new Date(holderBirthday.replace('/-/', '/')),
        ...
        personHandler(index) {
            ...
            Object.assign(this.holder.person, {
                ...
                defaultBirthday: new Date(insurant.birthday.replace('/-/', '/')),
        ...
        insurantHandler(index) {
            ...
            Object.assign(this.insurants[index], {
                ...
                defaultBirthday: new Date(person.birthday.replace('/-/', '/')),
            })
            this.updateChoose();
        },
        ...
        afreshInsurantHandler(item, insurant) {
            ...
            Object.assign(insurant, {
                ...
                defaultBirthday: new Date(insurantBirthday.replace('/-/', '/')),
            });
            return insurant;
        },
        ...
        confirmPickerHandler(values, index, type) {
            ...
            if(index === -3){
                ...
                if(type === 'birthday'){
                    ...
                    person.birthdayShowPicker = false;
                    if(new Date(values.replace(/-/, '/')).age() < 18){
                ...
                if(type === 'cardNo'){
                    ...
                    if(person.cardType && person.cardType.code == 1){
                        if(identityCardValidate(person.cardNo)){
                            ...
                            person.defaultBirthday = new Date(birthday.replace(/-/, '/'));
                ...
                if(type === 'cardNo'){
                    ...
                    if(insurant.cardType && insurant.cardType.code == 1){
                        if(identityCardValidate(insurant.cardNo)){
                            ...
                            insurant.defaultBirthday = new Date(birthday.replace(/-/, '/'));
        ...
        addHandler() {
            ...
            if(number < this.insurantNum){
                ...
                this.alertText = `被保人只能添加${number}个`;
                this.alertShow = true;
            }
        },
        ...
        checkFirstInsured() {
            ...
            for(let i = 0; i < insurants.length; i++){
                ...
                if(new Date(item.birthday.replace(/-/, '/')).age() >= 18){
            ...
            // 投保人验证
            {
                if(this.holder['type'] === 'enterprise'){
                    ...
                    if(!birthday){
                        ...
                        flag = false;
                    }else if(new Date(birthday.replace(/-/, '/')).age() < 18){
                        this.holder.person.birthdayErrMsg = '投保人年龄不满18周岁不符合投保要求';
                        flag = false;
                    }
                    if(genderShow && gender){
