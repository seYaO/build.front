...
(function(win){
    ...
    BaseMonitor.prototype = {
        ...
        isOnlineHost: function(){
            var host = location.host;
            return host.indexOf("www.ly.com") > -1
                || host.indexOf("wx.17u.cn") > -1
                  || host.indexOf("m.ly.com") > -1
                  || host.indexOf("ebk.17u.cn")>-1;
        },
        /**
         * @private
         * @func getLogStatus
        ...
        getLogStatus: function(){
            var isNotOnlineHost = !this.isOnlineHost(),
    ...
    $.extend(ErrorMonitor.prototype,{
         ...
         * @desc 日志接口的前缀
         */
        prefix: "//www.ly.com/dujia/ajaxhelper/commonhandler.ashx?action=FRONTPERFORMANCE&type=FrontLogErr",
        /**
         * @param _tmpl
         * @desc 错误信息模板
         */
        ...
        hostMap:{
            ...
            "(ebk\.t|ebk)\.17u\.cn":"EBooking"//pcebk
        },
        pageidMap:{
            "pc":{
                //www.ly.com/dujia
            ...
            "EBooking":{
                "\/home\/index[\/]?":"pc-ebk-index",//ebk跟团首页
                "\/productmanage\/([^\/]+)[\/]?":"ebooking-product",//ebk跟团产品
                "\/FlightManage\/([^\/]+)[\/]?":"ebooking-FlightIndex",//ebk机票
                "\/OrderResourceConfirm\/([^\/]+)[\/]?":"ebooking-OrderResourceConfirm",//ebk确认单
                "\/Comment\/([^\/]+)[\/]?":"ebooking-Comment"//ebk数据中心
            },
            "localhost": "localhost"
        },
        getBodyPageId: function(){
            return $("html").attr("trace");
        },
        getPageId: function(){
            var self = this,
                ...
                host;
            var bodyPageId = self.getBodyPageId();
            if(bodyPageId){
                return bodyPageId;
            }
        ...
        debug: function(){
            ...
            if(addr.indexOf("console")>-1){
                if(!win.console){
                    win.console = {};
                    win.console.log = function(){
                        self.console.apply(self,arguments);
                    };
                }
            }
            if(isOpen && isOpen[1]){
                window.addEventListener("load",function(event) {
                    var key = isOpen[1],
                        host = "http://10.14.84.206:1337",
                        url = host + "/vorlon.js/"+key,
                    ...
                    console.log("调试地址:"+host+"/dashboard/"+key);
                },false);
            }
        },
        console: function(){
            ...
            if(!self.consoleWin){
                $("body").append('<pre id="J_console" style="position:fixed;bottom: 0;left:0;width:100%;height: 32px;z-index: 1000000;background-color: #000;color: #0f0;overflow-y: scroll;"></pre>');
                self.consoleWin = document.getElementById("J_console");
                self.consoleMsg = [];
            }
            self.consoleMsg.push(msg);
            self.consoleWin.innerHTML = self.consoleMsg.join("<br/>");
            self.consoleWin.scrollTop = 16 * self.consoleMsg.length;
        }
    });
    ...
    $.extend(PerfMonitor.prototype,{
        ...
        performance: function(pageid){
            var self = this,
                url = "//www.ly.com/dujia/ajaxhelper/commonhandler.ashx?action=FRONTPERFORMANCE&type=FrontLogList",
        ...
        stat: function(type,param){
            var self = this,
                ...
                _data = {
                    host: location.host,
                    errortype: "stat-"+type,
                    pageid: self.pageid||self.getPageId()||"other",
                    url: encodeURIComponent(location.href),
                    jobnumber: $&&$.cookie&&$.cookie("jobnumber")||""
                },
        ...
        init: function(pageid){
            ...
            $(win).on("beforeunload",function(){
                ...
                var statStack = win.statStack;
                if(statStack){
                    self[statStack.method](statStack.type,statStack.data);
                }
            });
            //如果不需要在控制台显示
            if(!self.getLogStatus()){
    ...
    window.Monitor = window.monitorModule = new PerfMonitor();
    Monitor.init();
}(window));
