(function (win) {
    ...
    statperf.sendPerf = function () {
        ...
        // 页面停留时间
        data.stay = (new Date() - 1) - config.timestart;
        // 额外参数
        ...
        var statsend_data = Statsend.stringify(data)
        Statsend.send(statsend_url + '?key=frontPerf&frontlog=' + statsend_data);
    };
    // 

    STATKIT.getMonitor = function (hash) {
        if (!win.Monitor) {
            _.require('//file.40017.cn/dujia/cn/v/dujia/stat_monitor.js' + (hash || '20161219'));
        }
    };
    /****************业务方法********************/
    // 获取refid
    ...
    // 获取wxfid
    STATKIT.getWXid = function () {
        var cookie = _.cookie("WxUser") || "",
    ...
    // 获取NmemberId
    STATKIT.getNmemberId = function () {
        var a = _.cookie("nus"),
            c = "0";
        return a && (c = getParam(a, "userid"), c && "undefined" !== c || (c = "0")), c
    };
    //
    STATKIT.getAbtest = function () {
        var abtest = $.cookie('abTest');
        if (abtest == null) {
            abtest = this.getRandom(0, Math.pow(2, 32) - 1);
            $.cookie("abTest", abtest);
        }
        return abtest - 0;
    };
    //
    STATKIT.timeDiff = function () {
        var _timediff = -1;
    ...
    STATKIT.initial = function (options, callback) {
        ...
        win.setTimeout(function () {
            config.globalParam = win.__stat_param__ || {};
            config.spmParam = self.getSpotSPM();
            $.extend(config.param, { spm: config.spmParam.id }, config.globalParam);
            // 
            config.extraParam = {
                "refid": self.getRefId(),
    ...
    // 初始spot统计
    STATKIT.initSpotStat = function () {
        var self = this,
            config = self.config;
        var superWrapper = $(document);
        // 给所有统计热点添加事件
        superWrapper.on("mousedown", "a,[" + config.statinfo.point + "],[" + config.bizinfo.point + "],.J_Trace,.J_trace", function (e) {
    ...
    STATKIT.initStatUi = function () {
        ...
        $.ajax({
            dataType: "jsonp",
            url: 'http://10.101.40.81/board/checkstattoken?stattoken=' + statToken,
    ...
    STATKIT.getSpotSPM = function () {
        ...
        if (spmid) {
            spmList = spmid.split(config.spminfo.sep);
            spmObj = {
                id: spmid,
                list: spmList
            };
    ...
    // biz统计
    STATKIT.setBizTrack = function (elem) {
        var self = this,
            config = self.config,
        ...
        try {
            window._tcTraObj && _tcTraObj._tcTrackEvent(pageid, 'click', moduleid, trackVal);
        } catch (e) { }
    };
    // 发送热点信息
