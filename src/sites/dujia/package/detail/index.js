...
var Common = require("modules/common/0.1.0/index"),
    ...
    scrollspy = require("modules/scrollspy/0.3.0/index"),
    dialog = require("modules/dialog/0.1.0/index"),
    googleMap = require("modules/googleMap/0.2.0/index"),
    newdialog = require("dialog/0.2.0/dialog"),
    visa = require("modules/freePackage/visa/0.2.0/index"),
    Comments = require("comment/0.3.0/index"),
    Slidertoolbar = require("slidertoolbar/0.1.0/index"),
    Packaged = require("modules/freePackage/packaged/0.1.0/index"),
    lazyload = require("lazyload/0.1.0/index");;

var GroupCalendar = require("modules/datepicker/0.2.0/calendar-group");
var citytimer = null;
...
Group = {
    ...
    additionParam:[],
    tmpl: {
        routeDetail: require("./routerdetail.dot"),
        flyHotel: require("./flyandhoteldetail.dot"),
        insuranceDetail: require("./insurancedetail.dot"),
        moreHotel: require("./morehotel.dot"),
        priceBox: require("./pricebox.dot"),
        nearshop: require("./nearshop.dot"),
        shopPro: require("./shoppro.dot"),
        shopCity: require("./shopcity.dot"),
        findShop: require("./findshop.dot"),
        visa: require("./visa.dot"),
        roomDetail: require("./roomdetail.dot"),
        addPrice:require("./additionprice.dot"),
        tmplPreferential: require("./preferential.dot"),
        tmplTrafficMap: require("./trafficMap.dot")
    },
    content: {
    ...
    _init: function (cfg) {
        //从机票或者酒店返回时保证首屏可以看见loading，手动改变滚动条位置
        ...
        this.bookingEve();
        this.initslider();
        this.initGaodeMap();//初始化高德地图
        //推荐行程有天数
        if ($(".routeDays-box") && $(".routeDays-box").length) {
            this.initDayScroll();
        }
        //带有地图的推荐行程
        if($(".routeMap-box") && $(".routeMap-box").length){   
            this.initMapViewEvent();
        }
        //签证部分
        var countryId = $(".visa-content").attr("data-countryid");
        var visatypeid = $(".visa-content").attr("data-visatypeid");
        var regions = $(".visa-content").attr("data-regions");
        if(countryId && visatypeid && regions){
            var queryRegions = JSON.parse(regions);
            this.visaParam.CountryId = countryId;
            this.visaParam.VisaTypeId = visatypeid;
            this.visaParam.Regions = queryRegions;
            this.initvisa(countryId,queryRegions[0],visatypeid);
        }
        //this.mainEve();
        //触发$("#conlist")滚动
        $(window).scroll();
    ...
    initslider: function(){
        ...
        var slider = new Slidertoolbar({
            ...
            topMenu: [{
                icon: '<a href="//member.ly.com/"><div class="ico c-1"></div></a>',
                tooltips: '<a href="//member.ly.com/"><span class="ico-title">我的同程<i></i></span></a>',
                arrow: false
            }, {
                icon: '<a href="//member.ly.com/Member/MyFavorites.aspx"><div class="ico c-3"></div></a>',
                tooltips: '<a href="//member.ly.com/Member/MyFavorites.aspx"><span class="ico-title">我的收藏<i></i></span></a>',
                arrow: false
            }, {
                ideaClass: "udc-link",
                ...
                icon: '<a target="_blank" href="//livechat.ly.com/out/guest?p=7&lineid='+ self.lineId +'"><div class="ico c-5"></div></a>',
                tooltips: '<a target="_blank" href="//livechat.ly.com/out/guest?p=7&lineid='+ self.lineId +'"><span class="ico-title">在线客服<i></i></span></a>',
                arrow: false
            }],
            bottomMenu: [{
                icon: '<a href="//member.ly.com/"><div class="ico c-1"></div></a>',
    ...
    //行程地图事件
    initMapViewEvent: function(){
        var self = this;
        var $tab = $(".J_map_dialog .J_tab_item");
        $tab.on('click',function(e){
            e.stopPropagation();
            var _self = this;
            var $index = $(this).index();
            $(this).addClass('J_tab_item_active')
                .siblings().removeClass('J_tab_item_active');
            $(_self).parents(".J_map_dialog").find(".otherRoute-box-content").html('<div class="mapLoading"><i></i>正在加载中...</div>');
            var requestParms = {
                origin: null, //原点
            ...
            var latlng1={
                lng: $(this).parents(".J_map_dialog").siblings(".J_route_box_title").attr("data-prelng") || 0,
                lat: $(this).parents(".J_map_dialog").siblings(".J_route_box_title").attr("data-prelat") || 0
            }
            var latlng2={
                lng: $(this).parents(".J_map_dialog").siblings(".J_route_box_title").attr("data-nowlng") || 0,
                lat: $(this).parents(".J_map_dialog").siblings(".J_route_box_title").attr("data-nowlat") || 0
            }
            var startplace = $(this).parents(".J_map_dialog").siblings(".J_route_box_title").attr('data-startplace');
            var endplace = $(this).parents(".J_map_dialog").siblings(".J_route_box_title").attr('data-endplace');
            var place = {
                startplace: startplace,
                endplace: endplace
            }
            requestParms.origin = latlng1.lat + ',' +latlng1.lng;
            requestParms.destination = latlng2.lat + ',' +latlng2.lng;
            requestParms.alternatives = true;
            if($(this).hasClass('map-icon-bus')){
                requestParms.mode = "transit";
            }else if($(this).hasClass('map-icon-taxi')){
                requestParms.mode = "driving";
            }else if($(this).hasClass('map-icon-walk')){
                requestParms.mode = "walking";
            }
            self.searchLine(requestParms,$(_self).parents(".J_map_dialog").find(".otherRoute-box-content"),place);

        });
        //
        $(document).on("click",".J_open_detail_content",function(e){
            e.stopPropagation();
            if($(this).next('.J_OBRoute_way_content').hasClass('none')){
                $(this).html('收起线路详情<i></i>');
                $(this).addClass('content-active');
            }else{
                $(this).html('展开线路详情<i></i>');
                $(this).removeClass('content-active');
            }
            $(this).next('.J_OBRoute_way_content').toggleClass('none');
        });
        //打开弹窗信息
        $(".J_route_box_title").on("click",function(){
            $(this).parents('.route-maps').find(".J_map_dialog").addClass("none");
            $(this).next(".J_map_dialog").removeClass('none');
            $(this).next(".J_map_dialog").find(".route-box-content").html('<div class="mapLoading"><i></i>正在加载中...</div>');
            if($(this).hasClass('map-detail-title')){
                var requestParms = {
                    origin: null, //原点
                    destination: null, //目的地
                    mode: "" //搜索类型
                ...
                var latlng1={
                    lng: $(this).attr("data-prelng") || 0,
                    lat: $(this).attr("data-prelat") || 0
                }
                var latlng2={
                    lng: $(this).attr("data-nowlng") || 0,
                    lat: $(this).attr("data-nowlat") || 0
                }
                var startplace = $(this).attr('data-startplace');
                var endplace = $(this).attr('data-endplace');
                var place = {
                    startplace: startplace,
                    endplace: endplace
                }
                requestParms.origin = latlng1.lat + ',' +latlng1.lng;
                requestParms.destination = latlng2.lat + ',' +latlng2.lng;
                requestParms.alternatives = false;
                if($(this).hasClass("J_busWay")){
                    requestParms.mode = "transit";
                }else if($(this).hasClass("J_taxiWay")){
                    requestParms.mode = "driving";
                }else{
                    requestParms.mode = "walking";
                }
                self.searchLine(requestParms,$(this).next(".J_map_dialog").find(".route-box-content"),place,1);   
            }else{
                $(this).next(".J_map_dialog").find(".J_tab_item").eq(0).trigger("click");
            }
        })
        //关闭弹窗信息
        $(".J_close_dialog").on("click",function(){
            $(this).parents(".J_map_dialog").addClass('none');
        })
        //展开更多
        $(".J_intro_more").on("click",function(){
            $(this).parent().html($(this).parent().attr('data-content'));
            //$(this).addClass('none');
        });
        //天数滚动事件
        $("#conlist1 .con-map-day").scrollspy({
            curClass: "on",
            contentClass: ".map-day",
            topH: 80,
            ...
            scrollFn: function (navEl, isDown) {
                switch (isDown) {
                    ...
                    case 2:
                        ...
                        break;
                    case 1:
                        var topHeight = navEl.height() + 100;
        ...
        $(window).on('scroll', function () {
            var $mapTop = $("#routeInfo").offset().top;
            var $mapBot = $("#feeInfo").offset().top - $("#lineMap").height() - 100;
            var $left = $(".lineInfoContent").offset().left - $(window).scrollLeft() + $(".lineInfoContent").width();
            if ($mapTop < $(window).scrollTop() && $(window).scrollTop() < $mapBot) {
                $('.mapInfo').addClass('J_mapFixed');
                $('.mapInfo').css('left',$left);
            } else {
                $('.mapInfo').removeClass('J_mapFixed');
                $('.mapInfo').css('left','0');
            }
        });
        $(".J_click_days").on("click",function(){
            if(!$(this).hasClass("active")){
                $(this).addClass('active')
                    .siblings().removeClass("active");
                var latlng = JSON.parse(decodeURIComponent($(this).attr("data-latlng")));
                self.initGaodeMap(latlng);
            }
        });
    ...
    //查询线路,isDetailLine表示详细线路
    searchLine: function(requestParms,$target,place,isDetailLine){
        var self = this;
        var url = '/intervacation/ajax/maps/directions?origin='+ requestParms.origin +'&destination='+ requestParms.destination +'&mode='+ requestParms.mode +'&alternatives='+ requestParms.alternatives;
        $.ajax({
            url: url,
            dataType: 'jsonp',
            type: 'GET',
            success: function(directionsResult){
                if (directionsResult.Code == 4000) {
                    if (directionsResult.routes != undefined && directionsResult.routes.length > 0 && directionsResult.routes[0].legs != undefined && directionsResult.routes[0].legs.length != 0) {
                        
                        var travel_mode = '';
                        if(requestParms.travelMode == 'driving'){
                            travel_mode = 'OBRoute-taxiway-time';
                        }else if(requestParms.travelMode == 'walking'){
                            travel_mode = 'OBRoute-walkway-time';
                        }
                        var startplace = place.startplace || '';
                        var endplace = place.endplace || '';
                        var transitNameArr = [];
                        for (var i = 0; i <= directionsResult.routes.length - 1; i++) {
                            if(i < 10){
                                if (directionsResult.routes[i].legs != undefined && directionsResult.routes[i].legs.length != 0) {
                                    var TransitName = self.GetTransitNameByLegs(directionsResult.routes[i].legs);
                                    var itVal = directionsResult.routes[i].legs[0];
                                    transitNameArr.push(TransitName);
                                    directionsResult.transitName = transitNameArr;
                                    directionsResult.startplace = startplace;
                                    directionsResult.endplace = endplace;
                                    directionsResult.travel_mode = requestParms.mode;
                                    directionsResult.isDetailLine = isDetailLine;
                                }
                            }
                        }
                        
                        var allhtml = self.tmpl["tmplTrafficMap"](directionsResult);
                        $target.html(allhtml);
                    }else{
                        $target.html('<div class="mapLoading">抱歉，改交通方式暂无信息!</div>');
                    }
                }
            },
            error: function(){
        ...
        //                         directionsResult.startplace = startplace;
        //                         directionsResult.endplace = endplace;
        //                         directionsResult.travel_mode = requestParms.travelMode;
        //                         directionsResult.isDetailLine = isDetailLine;
        //                     }
        //                 }
        //             }
    ...
    //根据路段获取公交路线名称
    GetTransitNameByLegs: function(legs){
        if (legs == undefined) {
            return "";
        }
        var Name = "";
        for (var j = 0; j <= legs.length - 1; j++) {
            if (legs[j].steps != undefined) {
                for (var k = 0; k <= legs[j].steps.length - 1; k++) {
                    var item = legs[j].steps[k];
                    if (item.transit_details != undefined
                        && item.transit_details.line != undefined
                        && item.transit_details.line.short_name != undefined) {
                        Name += '<div class="OBRoute-way-start">'+ item.transit_details.line.short_name +'</div>'+ "<i></i>";
                    }
                }
            }
        }
    ...
    //初始化高德地图  切换出游时间变化地图
    initGaodeMap: function(latlng){
        var el = $(".mapInfo").find(".J_click_days");
        var thisLatlng;
        if(latlng){
            thisLatlng = latlng;
        }else{
            if(el.hasClass("active")){
                if(el.attr("data-latlng")){
                    thisLatlng = JSON.parse(decodeURIComponent(el.attr("data-latlng")));
                }
            }
        }
        if(thisLatlng && thisLatlng.length){
            var map = new AMap.Map('gaodemap',{
                resizeEnable: true,
                center: [thisLatlng[0].lng, thisLatlng[0].lat],
                zoom: 16
            });
            ...
            var markers = [];
            var image = '';    
            for(var i=0; i<thisLatlng.length; i++){
                image = '//file.40017.cn/dujia/images/assets/images/icon-mapMarker.png';
                //起点  终点区别一下
                if(thisLatlng.length > 1){
                    if(i == 0){
                        image = '//file.40017.cn/dujia/images/assets/images/icon-mapMarkerStart.png';
                    }else if(i == thisLatlng.length - 1){
                        image = '//file.40017.cn/dujia/images/assets/images/icon-mapMarkerEnd.png';
                    }
                }
                var lat = thisLatlng[i].lat;
                var lng = thisLatlng[i].lng;
                gaodepoints.push([lng,lat]);
                ...
                marker.setMap(map);
                //鼠标悬浮marker弹出自定义的信息窗体
                marker.content = thisLatlng[i].title;
            ...
            var polyline = new AMap.Polyline({
                path: gaodepoints,          //设置线覆盖物路径
                strokeColor: "#4c5a65", //线颜色
            ...
            googleLayer = new AMap.TileLayer({
                // 图块取图地址
                tileUrl: 'http://mt1.google.cn/vt/lyrs=m@142&hl=zh-CN&gl=cn&x=[x]&y=[y]&z=[z]&s=Galil',
                zIndex:100
            });
    ...
    lazyload: function(){
        if (Group.isInit) {
            var imgList = $(".J_route_img img").not("[data-img-loaded]");
            $("body").trigger("addElements",imgList);
        } else {
            $(".J_route_img img").lazyload({
                "data_attribute": "img",
                effect: 'fadeIn'
            });
            Group.isInit = true;
                ...
                if (data.Code == 4000) {
                    if (data.Data) {
                        ...
                        var _datepicker = new GroupCalendar({
                            ...
                            formatData: function (item) { //{day,date,value,enable}
                                for (var i = 0; i < data.Data.length; i++) {
                                    ...
                                    if(item && item.festival){
                                        item.value = '<div class="cal-disFes"><span class="festival"></span>' + item.day + '</div>';
                                    }else{
                                        item.value = item.day;
                                    }
                                    if ((new Date(v.Date.replace(/-/g,"/"))).format('yyyyMMdd') == (new Date(item.date.replace(/-/g,"/"))).format('yyyyMMdd')) {
                                        if (v.Price == 0) {
                                            dateprice = "实时计价";
                                        } else {
                                            dateprice = '¥' + v.Price;
                                        ...
                                        if(item && item.festival){
                                            if (v.IsLowest == 1) {
                                                item.value = '<div param_cal=' + param_cal + ' class="cal-sbox IsLowest">' + '<i></i><span class="festival"></span>' + item.day + '<div class="cal-price">' + dateprice + '</div></div>';
                                            } else {
                                                item.value = '<div param_cal=' + param_cal + '  class="cal-sbox"><span class="festival"></span>' + item.day + '<div class="cal-price">' + dateprice + '</div></div>';
                                            }
                                        }else{
                                            if (v.IsLowest == 1) {
                        ...
                        _datepicker.on("dayselect", function (o, d, date, vs, obj) {
                            var newdate = (new Date(date.replace(/-/g,"/"))).format('yyyy年MM月dd日');
                            $(".J_date").html(newdate);
                        ...
                        selday1 = (new Date(selday.replace(/-/g,"/"))).format('yyyy年MM月dd日');
                        $(".J_date").html(selday1);
    ...
    dealCalData: function (date) {
        for (var i = 0; i < CalendarData.length; i++) {
            if ((new Date(CalendarData[i].Date.replace(/-/g,"/"))).format('yyyyMMdd') == (new Date(date.replace(/-/g,"/"))).format('yyyyMMdd')) {
    ...
    initTip: function ($elem) {
        ...
        $dialog.tooltip({
            ...
            delay: 0, //延时隐藏时间，>0：鼠标离开后指定时间内隐藏，0：鼠标离开后立即隐藏,<0:不隐藏
            triggerEle: '.J_map_info_tips', //触发元素选择器
            triggerType: 'hover', //hover|click
    ...
    initEv: function () {
        ...
        $(".J_tmore").click(function () {
            ...
            if (self.hasClass('fold')) {
                ...
                $(self.parent().prev(".content-text")).css('max-height', '175px');
            } else {
    ...
    mainInit: function () {
        ...
        // var url = "http://10.100.156.179:8082/intervacation/api/PDynamicPackageProductDetail/PostDynamicSaleProduct";
    ...
    finalDeal: function (cfg) {
        ...
        data.flyParam.RoomCount = parseInt($(".roomItem input").val());
        ...
        self.mainEve();
        if(cfg.isback && cfg.isback == 1){
            var $flightH = $(".product-box").offset().top - 76;
            $('html,body').animate({scrollTop: $flightH}, 200);
        }
        if (data.Data) {
            self.priceScroll();
        }
            ...
            if (!_this.hasClass('haschlick')) {
                ...
                if ($hotel.attr("data-isdirect") == 0) {
                    ...
                    var _url = {
                        ...
                        "Adult": self.postData.Adult,
                        "ChildAges": self.postData.ChildAges,
                        "RoomCount": self.postData.RoomCount
                    };
                    var cctParam = $("#cctParam").val();
                    if (cctParam) {
                        _url.Platment = 8;
    ...
    mainEve: function () {
            ...
            if (self.hasClass('fold')) {
                ...
                if (!self.hasClass('hasClick')) {
                    ...
                    newurl = {
                        ...
                        "Adult": _self.postData.Adult,
                        "ChildAges": _self.postData.ChildAges,
                        "RoomCount": _self.postData.RoomCount,
                        "Hotel": postHotelData
                    };
                    var cctParam = $("#cctParam").val();
                    if (cctParam) {
        ...
        $(".btn-hotel").click(function () {
            var self = $(this);
            self.html('正在跳转中...');
            var content = '<div class="turn-warning fixed-warn1"><div class="data-loading"><div class="bg"></div><span>请稍候，正在为您跳转中...</span></div></div>';
            var config = {
                content: content,
                width: 500,
                title: '',
