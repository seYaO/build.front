...
if (pageType == 1 || pageType == 3) {
    stpSort = "scoreSort:1;lpCitySort:0;lcCitySort:0";
    orderSort = "orderCount:1;lpCitySort:0;lcCitySort:0";
    cmtSort = "cmtCount:1;lpCitySort:0;lcCitySort:0";
    asceSort = "tcPrice:0;lpCitySort:0;lcCitySort:0";
    descSort = "tcPrice:1;lpCitySort:0;lcCitySort:0";
} else {
    stpSort = "lpCitySort:0;lcCitySort:0;scoreSort:1";
    orderSort = "lpCitySort:0;lcCitySort:0;orderCount:1";
    cmtSort = "lpCitySort:0;lcCitySort:0;cmtCount:1";
    asceSort = "lpCitySort:0;lcCitySort:0;tcPrice:0";
    descSort = "lpCitySort:0;lcCitySort:0;tcPrice:1";
}
NewTag.prototype = {
    param: {
    ...
    initEve: function (cfg) {
        ...
        $(".list-box").hover(function () {
            var t = $(this);
            t.find(".hot_pic").removeClass("none");
            t.find(".hot_pic").addClass("show");
            t.find(".hot_maintitle").addClass("none");
            t.find(".hot_maintitle").removeClass("show");
            t.siblings().find(".hot_pic").removeClass("show");
            t.siblings().find(".hot_pic").addClass("none");
            t.siblings().find(".hot_maintitle").addClass("show");
            t.siblings().find(".hot_maintitle").removeClass("none");
        });
    },
    /**
        * @desc 产品hover效果
            ...
            success: function (data) {
                ...
                if (data.dest && data.dest.length > 0) {
                    for (var i = 0; i < data.dest.length; i++) {
                        var searUrl = "/dujia/search.aspx?src=" + conf.initCityName + "&dest=" + encodeURI(data.dest[i]) + "&prop=" + conf.prop;
    ...
    */
    ...
    getPage: function (cfg, totalpage) {
        ...
        var url = "/intervacation/api/PLabelSearchPage/GetProducts",
            tos = $("#hidTotalCount").val() || 0,
            tosl = Math.ceil(parseInt(tos, 10) / 40),
        ...
        var hasCondition = self.filter.hasCondition(),
            gpMode = 0;
        if ((cfg.prop == 1 || cfg.prop == 3) && searchType == 0) {
            dest = "";
            gpMode = hasCondition ? 0 : 1;
        }
        var param = $.extend({}, {
    ...
    filter: {
        ...
        initFilterEvOne: function (conf) {
            ...
            $(document).delegate(".sort-ul >li a","click", function (e) {
                ...
                if (conf.prop == 0 || conf.prop == 10) {
                    sortParam += ";scoreForLabel:1";
                }
                paramObj['stp'] = sortParam;
                self.removeCondition("stp");
                self.addCondition(paramObj);
                // 默认排序下如果是跟团自由行标签页且无筛选条件，则显示分组信息
                var filterConf = $.extend({}, conf || {}, {gpMode: 0});
                if (sortParam.match("scoreSort:1")) {
                    if ((conf.prop == 1 || conf.prop == 3) && searchType == 0) {
        ...
        _changeUrl: function(elArr,attr){
            if (searchType == 0) {
                ...
                $(elArr).each(function () {
                    ...
                    var reg1 = /(gentuan|zizhu|tag|lvyou)\-jingjia\/f/g,
                        reg2 = /(zizhu|lvyou|gentuan)\-jingjia\/(d|m|w)/g;
                    ...
                    var url1 = arr.join('/');
                    if(reg1.test(url)){
                        var _url = url.replace(/(\-(?:gentuan|zizhu|tag|lvyou)\-jingjia\/f)(\d+)/, function ($0, $1) {
                            return $1 + id;
                        });
        ...
        _initFilter: function (conf,callback) {
            ...
            self.ajax(self._param_).then(function (data) {
                $(".loading").hide();
                if (data.Data == null || data.Data.TotalCount === 0) {
                    $("#tagList").empty();
                    $("#J_LablePager").empty();
                    $("#J_NoLine").show();
                    return;
                ...
                self.renderFilter(data.Data, function () {
                    // self._changeUrl(".J_lineCity>a");
                    if (self._condition_.stp) {
                        if(self._condition_.stp.match("orderCount:1")) {
                            $(".sort-ul li").removeClass("cur");
                            $(".sort-ul li").eq(1).addClass("cur");

                        }
                        if(self._condition_.stp.match("cmtCount:1")) {
                            $(".sort-ul li").removeClass("cur");
                            $(".sort-ul li").eq(2).addClass("cur");
                        }
                        if(self._condition_.stp.match("tcPrice:1")) {
                            $(".sort-ul li").removeClass("cur");
                            var _par =$(".sort-ul li").eq(3);
                            _par.addClass("cur");
                        } else if(self._condition_.stp.match("tcPrice:0")) {
                            $(".sort-ul li").removeClass("cur");
                            var _par =$(".sort-ul li").eq(3);
            ...
            for (var i in self._condition_) {
                if (i === "stp") {
                    ...
                    if (pageType == 0 || pageType == 10) {
                        self._condition_[i] += ";scoreForLabel:1";
                    }
                } else {
                    self._condition_[i] = "";
                }
        ...
        hasCondition: function(){
            ...
            for (var k in ret) {
                ...
                if((ret[k] || ret[k] === 0) && k !== "stp"){
                    hasCondition = true;
                    return hasCondition;
                }
                if (k === "stp") {
                    if (!ret[k].match("scoreSort:1") && (ret[k] || ret[k] === 0)) {
                        hasCondition = true;
                        return hasCondition;
                    }
                }
