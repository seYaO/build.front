...
(function($){
    ...
    Common.isIOS = /iP(ad|hone|od)/.test(navigator.userAgent);
    ...
    Common.initCloseEvent = function(){
        ...
        $('.header-search').on('click',function(){
            var dataKey = 'package';
            $('#mainPage').attr('line-type') === '跟团' ? dataKey='package' : dataKey='travel';
            //console.log((window.__jurl__ || 'http://m.ly.com/dujia')+dataKey+'/destination')
            Common.goTo((window.__jurl__ || '/')+dataKey+'/destination');
        });
    ...
    Common.getLocation = function(cfg){
        ...
        navigator.geolocation.getCurrentPosition(function(position){
            ...
            var url = "http://api.map.baidu.com/geocoder/v2/?ak=R1BS15aSlKUTpc0YG6GbDk7Q&output=json&pois=0&location="+loc;
    ...
    Common.resetHeadIndexBtn = function () {
        $(document).on("click", ".header-menu-btn", function (e) {
            ...
            $menu.css("top", $obj.offset().top + $obj.height() + 9 + "px");
            $menu.addClass("open");
            //e.preventDefault();
    ...
    Common.init = function(){
        ...
        Common.setRefId();
        Common.XiaoGuTop();
        var FastClick = require("modules/utils/fastclick/index");
        /* jshint -W031  */
    ...
    Common.wishPop = function () {
        var wishpop = require("utils/wish/index");
        new wishpop({
            wishUrl: "http://www.ly.com/zhuanti/wishlist/",//许愿地址
            fromUrlRege: "m.ly.com/dujia", //来源页面
            //fromUrlRege: /\.ly\.com/i, //来源页面
            triggerUrlRege: 'touch-detail;touch-list;touch-index'  //触发页面
        });
    };
     ...
     *
     */
    Common.XiaoGuTop = function(){
        var refid = $.cookie('17uCNRefId');
        if(refid == '193362614'){
            $('.page-header').hide();
        }
    };
