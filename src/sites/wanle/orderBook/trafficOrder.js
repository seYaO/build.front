...
var Details = {
    init: function () {
        ...
        var get_num = getURLParameter().currentnumber;
        var get_priceId = getURLParameter().currentpriceid;
        $("#CurrentNumber").val(get_num);
        $("#CurrentPriceId").val(get_priceId);
        $(".num_box input").val(get_num);
        if(pageConfig.VisitorInfoNeed == 2){
            $("#needPassengerNum").html(get_num);
        }else if(pageConfig.VisitorInfoNeed == 1){
        ...
        Details.GetWanLePreferentialInfoAjaxAndRenderPage(); //优惠信息
        Details.BindLogin();
        //buildFee(); //计算费用
        Details.BillAction();
        Details.ClickBill();
        ...
        Details.NoneId();
        Details.isShowChild();
        Details.freeChildSet();
        //Details.hidSexBirthday();
        Details.isAdult();
    ...
    ShowOrHideDiscountList: function () {
        ...
        var originFilter = {
            ...
            LineStartDate: function (val) {
                var oneCal = $('#mainCal1').attr("value");
                if (oneCal == "") {
                    return true;
                } else {
        ...
        var filterRuleList = _.filter(allRuleList, function (r) {
            return FilterData([r], originFilter);
        });
    ...
    CalculateFinalAmout: function (curRuleObj) {
        ...
        if (curRuleObj && curRuleObj.PreferentialMode == 1 && curRuleObj.PreferentialPriceType == 0 && (curRuleObj.PreferentialType == 0) && price >= curRuleObj.PreferentialPrice) {
            ...
            var startDate = $("#mainCal1").attr("value");
    ...
    GetWanLePreferentialInfoAjaxAndRenderPage: function () {
        $.ajax({
            ...
            // url: window.__mod__ +"/wanle/api/WanleProduct/GetWanLePreferentialInfo?LineId="+ pageConfig.productId +"&Platment=0&PublicPlatmentId=1",
    ...
    BindPostAddress: function () {
        ...
        $("#provinceqg").on("click",function(){
            ...
            $("#trictList").addClass("none");
            $.ajax({
                url: "http://www.ly.com/wanle/api/WanleProduct/GetProvinceList",
                dataType: "json",
                success: function (data) {
                    data = data.Data;
        ...
        $(document).on("click","#proList li",function(){
            ...
            $("#proList").addClass("none");
            $.ajax({
                url: "http://www.ly.com/wanle/api/WanleProduct/GetCityListByProvinceId?id="+$("#provinceqg").attr("value"),
                dataType: "json",
                success: function (data) {
                    data = data.Data;
                    $("#cityList").html("");
        ...
        $(document).on("click","#cityList li",function(){
            ...
            $.ajax({
                url: "http://www.ly.com/wanle/api/WanleProduct/GetCountyListByCityId?id="+$("#city").attr("value"),
                dataType: "json",
                success: function (data) {
                    data = data.Data;
                    $("#trictList").html("");
    ...
    GetAllLinkManAjaxAndRenderPage: function () {
        $.ajax({
            // url: window.__mod__ + "/wanle/AjaxHelperWanLe/GetContanctList?MemberId=" + pageConfig.memberId,
            url: "http://www.ly.com/wanle/api/WanleProduct/GetContanctList?MemberId=" + pageConfig.memberId,
            dataType: "jsonp",
            success: function (data) {
    ...
    AddPassenger: function (passengerObj,param) {
        ...
        Details.LoginShowOrHide();
        //Details.hidSexBirthday();   //隐藏或显示多余的性别和出生日期
    ...
    GetFirstEmptyLinkMankId: function (selector) {
        ...
        $(finalSelector).each(function () {
            ...
            if (curId.indexOf('self') > -1) {
                ...
                var IsCheckIsAllStringEmpty = CheckIsAllStringEmpty.call(perInputData);
    ...
    //    $(".passenger-info-table").each(function(selector){
    ...
    SubOrderAjax: function () {
        ...
        //是否需要儿童座椅取值
        if($(".free_get label").hasClass("on_check")){
            var getChildMore = 1
        }else{
            var getChildMore = 0
        }
        ...
        //var InsuranceCount = $(".inputbox").val();          //保险份数
        //意外险险种Id
        var accident_id = 0;
        ...
        $.ajax({
            ...
            //url:"http://10.101.40.16:8045/Wanle/AjaxHelperWanLe/WanLeSubOrder",
            type: 'POST',
            data: {
    ...
    bindAddBtn: function () {
        ...
        $('#sel_car_num').delegate('.chose_peo .num_box a:last-child', 'click', function () {
        ...
        $('#sel_car_num').delegate('.chose_peo .num_box a:first-child', 'click', function () {
            var fxFlag = true;
        ...
        $('#sel_child_num').delegate('.chose_child .num_box2 a:last-child', 'click', function () {
            if($(this).hasClass("add_btn_false")){
                return;
            }
            var inputBox = $(this).parent().find('.inputbox_child')[0];
            var curNum = parseInt($(inputBox).val()) || 0;
            $(inputBox).val(curNum + 1);
            $(".first_insure .t4").html(curNum + 1);
            Details.checkSubOrAddBtnCanUse();
        ...
        $('#sel_child_num').delegate('.chose_child .num_box2 a:first-child', 'click', function () {
            if($(this).hasClass("sub_btn_false")){
                return;
            }
            var inputBox = $(this).parent().find('.inputbox_child')[0];
    ...
    checkSubOrAddBtnCanUse: function () {
        var sumPep = 0;
        $(".car_num input").each(function () {
            var curNum = parseInt($(this).val()) || 0,
            //var curNum = parseInt($(this).val()) || parseInt($("#BookMin").html()),
                curMax = parseInt($(this).attr('max')) || 0,
                curMin = parseInt($(this).attr('min')) || 0,
        ...
        $(".child_num input").each(function () {
            ...
            //var curNum = parseInt($(this).val()) || parseInt($("#BookMin").html()),
                ...
                lastElem = $("a:last-child", $(this).parent()),
                firstElem = $("a:first-child", $(this).parent()),
                c_num = parseInt($(".inputbox").val()) || 0;
            sumPep = sumPep + curNum;
            $(this).val(curNum);
            //计算相加总和
            var totalNum = curNum + c_num;
        ...
        if(sumPep >pageConfig.minOrderPerson && sumPep <= pageConfig.maxOrderPerson){
            $(".car_num .inputbox").each(function () {
    ...
    submitBooking: function (e) {
        ...
        if (pageConfig.hidProductCategory == "92") { //wifi产品
            ...
            var days = 1;
            days = (NewDate(mainCal2.val()) - NewDate($("#mainCal1").attr("value"))) / (24 * 60 * 60 * 1000) + 1; // 如 7号到9号  其实是三天 所以+1
    ...
    BindKeyUp: function () {
        ...
        $(".inputbox_child").on("blur",function(){
            Details.checkSubOrAddBtnCanUse();
            buildFee();
            //Details.isShowChildMoney();
    ...
    moreAddress:function(){
        //是否显示更多地址按钮
        if($(".getback .productInfo li .receiveplace").length > 2 || $(".getback .productInfo li .returnplace").length > 2){
...
//失去焦点直接进行验证
    ...
    BindPostInfoAjax: function () {
        $.ajax({
            ...
            success: function (data) {
                data = data.Data;
                if (data.Status.IsSuccess && data.AddressList != null) {
                    pageConfig.AddressList = data.AddressList;
    ...
    OutPhone:function(){
        ...
        $(".user_info input").on("focus",function(){
    ...
    freeChildSet:function(){    //选择更多儿童座椅
        $(".free_get input").on("click",function(){
            if($(this).parent().hasClass("on_check")){
                $(this).parent().removeClass("on_check");
                //$(".more_child").addClass("none");
                $(".free_get span").html("已免费赠送儿童座椅一张，可勾选使用");
                $(".free_get span").css("color","#999");

                //$(".inputbox_child").val("0");
                //
                //Details.checkSubOrAddBtnCanUse();
                //buildFee();
                //Details.isShowChildMoney();
                //Details.ShowOrHideDiscountList();
                //$(".hid_child_set").addClass("none");
            }else{
                $(this).parent().addClass("on_check");
                $(".free_get span").html("该儿童座椅免费提供的");
                $(".free_get span").css("color","red");

                if($("#IsHasChildMore").val() == 1){
                    $(".more_child").removeClass("none");
                }else{
                    $(".more_child").addClass("none");
                }
    ...
    isShowChild:function(){
        if($("#IsFreeChildSeat").val() == 1){                   //有免费座椅
            $(".special_server").removeClass("none");
            $(".free_get label").css("display","inline-block");
            $(".free_get span").removeClass("none");
            $(".free_get span").text("免费领取儿童座椅一张");
        }else{                                                    //没有免费座椅
            if($("#IsHasChildMore").val() == 0){                //没有收费座椅&&没有免费座椅
                $(".special_server").addClass("none");
                $(".more_child").addClass("none");
            }else{                                                //有收费座椅&&没有免费座椅
                $(".special_server").removeClass("none");
                $(".free_get").removeClass("none");
                $(".more_child").removeClass("none");
                $(".free_get label").css("display","none");
                $(".free_get span").addClass("none");
                $(".free_get font").addClass("set_zhan");
                $(".free_get font").text("儿童座椅");
            }

        }
    },
    //判断是否显示儿童座椅费用信息
    isShowChildMoney:function(){
        if($(".more_child").hasClass("none")){
            $(".hid_child_set").addClass("none");
        }else{
            $(".hid_child_set").removeClass("none");
        }
    },
    //保险交互事件
    insure_event:function(){
        ...
        $(".accident_list .insure_list .t2 .no_more").hover(function(){
            ...
            //$(this).parent().parent().find(".insure_detail").css({left:l+"px",top:t+"px"});
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(data.Data.ProductInsuranceList.length == 0 ){        //说明没有意外险
                    ...
                    //$(".hid_accidentInsure").removeClass("none");
                    $(".hid_cancelInsure").removeClass("none");
                    $("#HasInsurance").val("True");
                }

                if(pageConfig.hidProductCategory == "111"){              //是一日游
                    if(data.Data.ProductInsuranceList.length > 0){           //有意外险
                        $(".player_info .passenger-info-table input").addClass("inputBeforeSubmitChk");
                    }else{
                        $(".player_info .passenger-info-table input").removeClass("inputBeforeSubmitChk");
                    }
                ...
                //if(data.Data.ProductInsuranceList.length > 0 && $("#VisitorInfoNeed").val() != "0"){
                //    $(".player_info .passenger-info-table input").addClass("inputBeforeSubmitChk");
                //}else{
                //    $(".player_info .passenger-info-table input").removeClass("inputBeforeSubmitChk");
                //}
                ...
                $(".accident li").each(function(){       //默认选中意外险
                    if($(this).find(".button_right").attr("isShow") == "true"){
                        $(this).find(".button_right").addClass("has_right");
                    }
                });
...
function buildFee(fxFlag) {
    ...
    $(".inputbox").each(function (index, elem) {
        ...
        var priceTypeStr = $(".productInfo .p_title .list_right").text() || '';
        ...
        isCancelPrice = eachPriceSum;     //判断取消险的总价 =  门票总价      意外险总价 + 不要了
        ...
        //判断总额是否有押金
        if($(".pay_online").hasClass("selected")){
            totalPriceSum = eachPriceSum + eachdeposit;
        }else{
            //判断总额是否有意外险和取消险
            //if($("#HasInsurance").val() == "True"){
            totalPriceSum = eachPriceSum  + cancelPrice;
            //}else{
            //    totalPriceSum += eachPriceSum;
            //}
        ...
        if (feeinfoDl.length == 0) {
            var tmpl = '<dl class="calculPri_{priceId}">' +
                ...
                '</span>' +
                '</dd>' +
                '<div class="hid_child_set">'+'<dt>儿童座椅</dt>' +
                '<dd class="child_set">' +
                '<span class="font_W boxL">' +
                '<span class="">' +'￥<strong class="d_thisPrice">'+ 0 +'</strong>' + '</span>' +
                'x' +
                '<b class="d_listpeo">0</b>' + '=' + '<b class="d_orderPrice colF60">'+ 0 +"元"+'</b>' +
                '</span>' +
                '</dd>' +
                '</div>'+
                '<div class="hid_accidentInsure none">'+'<dt">意外险</dt>' +
            ...
            $(".orderPrice").attr("adult_price",eachPriceSum);
            ...
            elem.find(".listpeo").html("" + thisNum);
            //elem.find(".thisDaysNum").html("(" + pageConfig.orderDayNum + "天)");
            elem.find(".thisPrice").html(thisPrice);
    ...
    var eachChildSetMoney = 0;
    var getChildSetMoney = parseInt($(".num_box2 input").attr("data-price-child"));
    $(".inputbox_child").each(function(index, elem){
        var thisNum = parseInt($(elem).val()) || 0;
        eachChildSetMoney = getChildSetMoney * thisNum;

        $(".hid_child_set .d_thisPrice").html(getChildSetMoney);
        $(".hid_child_set .d_listpeo").html(thisNum);
        $(".hid_child_set .d_orderPrice").attr("s_price",eachChildSetMoney);
        $(".hid_child_set .d_orderPrice").html(eachChildSetMoney + "元");
    });

    tmplArr.length > 0 && feeinfo.html(tmplArr.join(''));


    totalPriceSum = parseInt($(".hid_child_set .d_orderPrice").html().split("元")[0]) + parseInt($(".orderPrice").html().split("元")[0]) + parseInt($(".cancel_orderPrice").html().split("元")[0]);
    ...
    if($(".inputbox_child").val() == 0){
        $(".hid_child_set").addClass("none");
    }
}

//对象变 queryString  wmg
function getParamStr(joinChar) {
...
function CheckIsAllStringEmpty() {
    var b = true;
    for (var i in this) {
