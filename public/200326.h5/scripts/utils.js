 ...
 * 蒙层/需要页面配合
 */
window.utils.popFn = function (val) {
    $(".showErr").html(val);
    $(".showErrBox").show();
    setTimeout(function () {
        $(".showErr").html("");
 ...
 * 小程序分享
 */
window.utils.setMiniappShare = function (options) {
    var spm = options.spm;
    var refid = options.refid;
    var url = encodeURIComponent(window.__configs.shareInfo.shareUrl + "?isxcx=1&spm=" + spm + "&refid=" + refid);
    var path = "https://wx.17u.cn/wl/api/redirect?redirect_uri=" + url;
    wx.miniProgram.postMessage({
        data: {
            shareInfo: {
                currentPath: location.host + location.pathname, //当前页面路径，不需要参数
                title: window.__configs.shareInfo.miniappTitle,
                path: path, //默认当前页面路径
                imageUrl: window.__configs.shareInfo.miniappImageUrl //支持PNG及JPG，不传入 imageUrl 则使用默认截图。显示图片长宽比是 5:4
            }
        }
    });
}
