    ...
    methods: {
                    ...
                    switch (index) {
                        ...
                        case 2:
                            if (data.List.length > 0) {
                                that.newBKlist[0].list = data.List.slice(0, 3)
                            } else {
                        ...
                        case 3:
                            if (data.List.length > 0) {
                                that.newBKlist[1].list = data.List.slice(0, 3)
                            } else {
                                that.newBKlist[1].list = ''
                            }
                            break;
                        case 4:
                            // that.sectionData2 = data.List[0]
                            // that.sectionData3 = data.List.slice(1, 6);
            ...
            function results(datas) {
                ...
                if (datas.length > 0) {
                    ...
                    datas.forEach(function (item) {
                        ...
                        if (labs[0] && labs[0] == 1) {
                            list.push({ ...item, label: labs[1], score: new Array(Number(labs[2])) })
                        } else {
                            list.push(item)
                        }
                    })
        ...
        bcChangeTab: function (index) {
            if (index != this.bcTabIndex) {
                ...
                this.sectionData6 = this.newBClist[this.bcTabIndex].list;
                this.sectionData9 = this.newBClist[this.bcTabIndex].moreList;
            }
        },
        windowLocationHref: function (type, item) {
            if (type == 'order') {
                window.location.href = this.goOrder(item.SceneryId, item.BCTTicketId)
            } else if (type == 'detail') {
                window.location.href = this.getLink(item.SceneryId, item.Wurl, item.Murl)
            }
        ...
        getLink: function (sceneryId, wurl, murl) {
            if (this.isTc) {
                return 'tctclient://react/page?projectId=117001&page=Detail&sceneryId=' + sceneryId + '&spm=' + this.spm
        ...
        goOrder: function (sceneryId, priceId) {
            if (this.isTc) {
                return 'tctclient://react/page?projectId=117001&page=Order&sid=' + sceneryId + '&policyid=' + priceId

            } else if (this.isWx) {
                return '//wx.17u.cn/scenery/booking/newbook1_' + sceneryId + '_' + priceId + '.html?source=1&spm=5.29517.29517.1' + this.addHtml;
            } else {
                return '//m.ly.com/scenery/booking/newbook1.html?sceneryId=' + sceneryId + '&priceid=' + priceId + '&spm=5.29517.29517.1' + this.addHtml;
            }
        },
        /**
         * 红包模块
