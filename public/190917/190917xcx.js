...
var app = new Vue({
    ...
    methods: {
                    ...
                    switch (index) {
                        ...
                        case 66:
                            ...
                            data.List.forEach(function (item) {
                                if (item.SceneryId == 0 || item.SceneryId == 2) {
                                    list.push(item)
                                }
                            })
        ...
        getTimeFn: function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (new Date(that.nowTime) > new Date(that.timeEnd + ' 59:59:59')) {
        ...
        countDownFn: function (close) {
            var timeStr = this.nowTime.slice(0, -9);
        ...
        //链接跳转
        getLink: function (sceneryId, index) {
            var spm = this.spm.slice(0, -1) + index
            wx.miniProgram.navigateTo({
                url: "/page/top/pages/scenery/detail/detail?sid=" + sceneryId + "&wxspm=" + spm + "&wxrefid=" + this.refid
            });
        },
        goOrder: function (sceneryId, priceId, index) {
            var spm = this.spm.slice(0, -1) + index
            wx.miniProgram.navigateTo({
                url: "/page/top/pages/scenery/order/order?sid=" + sceneryId + "&policyid=" + priceId + "&suppliertype=0&wxspm=" + spm + "&wxrefid=" + this.refid
            });
        },
